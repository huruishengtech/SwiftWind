...
# train.py for Observing system simulation experiments (OSSEs)
...
from dataloaders_osse import SwiftWind_train_val_dataloaders
...
def main():
    ...
    if not data_config['test']:
        ...
        cbs = [ModelCheckpoint(monitor="val_loss", filename="val-{epoch:02d}", every_n_epochs=1),
               EarlyStopping(monitor="val_loss", min_delta=1e-5, patience=200, mode="min")]
